...
_Java Syntax VS Python Syntax_

1) All the code is in a class, specifically in its "main" method
2) "and" --> "&&"
2) carets "{}" to tell level instead of indentation
3) for loop has 3 parts: initialization (int i = 1), stopping condition (i < 100), and the incrementation (i++)
4) Have to declare/initialize variable types before giving them a value
5) if / else statements are very similar to Python
